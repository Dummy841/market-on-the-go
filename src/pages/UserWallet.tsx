...
import { Wallet, ArrowUpRight, ArrowDownLeft, Clock, ArrowLeft, Plus } from 'lucide-react';
...
const UserWallet = () => {
  ...
  const fetchWalletData = async () => {
    ...
    try {
      ...
      const {
        data: walletData,
        error: walletError
      } = await supabase.from('user_wallets').select('balance').eq('user_id', user.id).maybeSingle();
      if (walletError) {
      ...
      let query = supabase.from('user_wallet_transactions').select('*').eq('user_id', user.id).order('created_at', {
        ascending: false
      }).limit(100);
      if (filterFrom) {
  ...
  return <div className="min-h-screen bg-background pb-[env(safe-area-inset-bottom)]">
      {/* Header */}
          ...
          <Card className="bg-gradient-to-r from-primary to-orange-500 text-white">
            <CardContent className="p-6">
              <div className="flex flex-col items-center text-center">
                ...
                <Button onClick={() => setShowAddMoneyModal(true)} className="mt-4 bg-white text-primary hover:bg-white/90" size="sm">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-2 p-2 bg-muted/50 rounded-lg overflow-x-auto">
              ...
              <Input id="filterFrom" type="date" value={filterFrom} onChange={e => setFilterFrom(e.target.value)} className="h-7 w-[100px] text-[10px] px-1.5" />
              <Label htmlFor="filterTo" className="text-[10px] text-muted-foreground flex-shrink-0">To</Label>
              <Input id="filterTo" type="date" value={filterTo} onChange={e => setFilterTo(e.target.value)} className="h-7 w-[100px] text-[10px] px-1.5" />
              <Button onClick={handleApplyFilter} size="sm" className="h-7 px-2 text-xs flex-shrink-0">
                Apply
              </Button>
              {(filterFrom || filterTo) && <Button onClick={handleClearFilter} variant="outline" size="sm" className="h-7 px-2 text-xs flex-shrink-0">
                  Clear
                </Button>}
            </div>

            {loadingTxns ? <div className="text-center py-8 text-muted-foreground">Loading...</div> : transactions.length === 0 ? <div className="text-center py-8 text-muted-foreground">
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input id="amount" type="number" placeholder="Enter amount" value={addAmount} onChange={e => setAddAmount(e.target.value)} min="1" className="text-lg" />
            </div>
            <div className="flex flex-wrap gap-2">
              {quickAmounts.map(amt => <Button key={amt} variant="outline" size="sm" onClick={() => setAddAmount(amt.toString())} className={addAmount === amt.toString() ? 'border-primary bg-primary/10' : ''}>
                  ₹{amt}
                </Button>)}
            </div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={handleAddMoney} disabled={isAddingMoney || !addAmount} className="bg-primary hover:bg-primary/90">
