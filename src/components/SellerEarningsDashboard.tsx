...
const SellerEarningsDashboard = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('orders')
        .select('id, created_at, total_amount, status, items, seller_status')
  ...
  const calculateEarnings = (ordersData: Order[]) => {
    ...
    const earningsArray = Array.from(earningsByDate.values()).sort(
      (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
    );

    setDailyEarnings(earningsArray);
    setTotalStats({ totalOrders, totalRevenue, totalEarnings });
  };
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  After {seller?.franchise_percentage || 0}% deduction
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-muted-foreground">No delivered orders in this date range.</p>
          ) : (
