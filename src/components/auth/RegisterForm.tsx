...
export const RegisterForm = ({ isOpen, onClose, onSuccess, initialMobile }: RegisterFormProps) => {
  ...
  const resetForm = () => { setStep('register'); setName(""); setMobile(""); setSessionId(""); setOtp(""); setResendTimer(0); setError(""); setAgreedToTerms(false); setShowTerms(false); setShowPrivacy(false); isVerifyingRef.current = false; };
        ...
        <DialogContent className="!max-w-full !w-full !h-full !max-h-full !rounded-none m-0 p-0 flex flex-col z-[10001]">
          ...
          >
            ...
            <div className="mt-8 pb-4">
              {termsScrolledToEnd ? (
                <Button onClick={() => setShowTerms(false)} className="w-full h-12 text-base font-semibold">Close</Button>
              ) : (
          ...
          >
            ...
            <div className="mt-8 pb-4">
              {privacyScrolledToEnd ? (
                <Button onClick={() => setShowPrivacy(false)} className="w-full h-12 text-base font-semibold">Close</Button>
              ) : (
